...
## Reader Comfort And Surface Hygiene Gate

sidecarの自己申告だけで通過させず、画像を拡大して、身につける物、口に触れる物、手で扱う私物と接地面の関係を確認する。

- `readerComfort.reviewedBy: agent:visual-editor`、空配列の `readerComfort.hygieneSensitiveItemsOnFloor`、具体的な `readerComfort.placementNotes` がsidecarにあることを確認する。
- 食品、飲料容器、書籍、衣類、ハンカチ、タオル、財布、鍵、携帯電話、腕時計、アクセサリー、身だしなみ用品が、路面、地面、駅ホーム、玄関土間・たたき、トイレ床、汚れが見える床へ直接置かれている場合は通過させない。
- 清潔な室内床か玄関土間か判別できない場合も、読者が不潔と受け取る余地を優先して差し戻す。
- 靴や屋外用の鞄の自然な床置きは許容できるが、鞄の中身を周囲の床へ並べた構図は差し戻す。
- 不一致があれば `kotatsu:revise` と `agent:visual-editor` に戻し、問題の物、接地面、代替となる棚・ベンチ・トレー等をコメントへ具体的に記録する。
