...
Recovery has a separate decision path but keeps the normal quality gates. Any daytime scheduled run can become a bounded recovery coordinator for the oldest delayed article and dispatch one role-specific subagent at a time. Production workers still return through a separate managing-editor worker before the next role, while a gate-complete Delivery failure can go directly to a publisher worker. The session stops after one article, 90 minutes, six workers, a quality decision, an external blocker, or 19:00 JST; GitHub remains the durable fallback queue. Passed copy and approved visuals remain valid only when the delivery move stays within seven days and the same month and no reader-facing old date exists. Protected future dates never move as a cascade, and no extra high-frequency or late-night automation is added.
