...
- Day 1 16:00 進行編集: ライター成果PRを確認し、`main` に反映済み、または安全に反映できるものだけビジュアル編集へ整理する
...
次担当への共有完了は、PR作成ではなく `main` への反映で判断する。

- 月刊号設計 -> ライター: 正式計画が `main` に反映済みであること。PRだけ、ローカルだけ、候補メモだけでは不可。
- ライター -> ビジュアル編集: 記事PRが `main` に反映済み、または進行編集が同時に安全にマージできること。Draft PR、CI失敗、未マージPRだけの成果物は不可。
- ビジュアル編集 -> 校正: AI生成ビジュアル、alt、caption、metadata、避けた要素が確認でき、成果PRが `main` に反映済みまたは安全にマージできること。
- 校正 -> 公開担当: 校正結果が公開可能で、修正PRがある場合は `main` に反映済みまたは安全にマージできること。
- 公開担当 -> done: 公開ゲート、CI、build、可能な範囲のスクリーンショット確認が通っていること。

成果物がPR上にだけ存在する状態で次工程へ渡す場合は、次担当の自動化が明示されたブランチをcheckoutできる設計が必要になる。現行運用では各担当が通常の作業ディレクトリを前提に動くため、進行編集は原則として `main` 反映後に次工程へ渡す。

## `kotatsu:ready` の責任

`kotatsu:ready` は「次担当が迷わず着手できる状態」を示す。原則として進行編集が最終管理者として付与、維持、解除を判断する。ユーザー、編集長、各担当エージェントが `kotatsu:ready` を提案することはできるが、前工程の不足、担当label、milestone、公開週、素材条件、必要成果物の `main` 反映を確認して確定するのは進行編集の役割とする。
...
- 公開対象の計画、記事、画像、校正結果が `main` に反映済みで、CI/CDが同じ内容をビルドできる
