...
静かな生活感は、私物を無造作に床へ置くことで表現しない。とくに日本の玄関土間・たたきは屋外側として受け取られやすいため、衛生面の説明がなくても画像だけで不快感を生む配置を避ける。

- 食品、飲料容器、書籍、衣類、ハンカチ、タオル、財布、鍵、携帯電話、腕時計、アクセサリー、身だしなみ用品を、路面、地面、駅ホーム、玄関土間・たたき、トイレ床、汚れが見える床へ直接置かない。
- 靴や屋外用の鞄は床置きが自然な場合に限って許容する。鞄の中身を床へ並べない。
- 清潔な室内床に見せる意図でも、玄関土間や屋外と誤認しうる場合は不採用とする。棚、玄関ベンチ、テーブル、トレー、敷き布など用途が明確な面を使う。
- 生成後に画像を拡大し、物と接地面を目視する。配置が曖昧な場合は安全側に倒して再生成する。
...
"readerComfort": {
  ...
  "placementNotes": "本、ハンカチ、水筒、財布、鍵は玄関ベンチ上の布製トレーに置かれ、床へ直接触れていない。"
}
```

`hygieneSensitiveItemsOnFloor` は実画像で直接床置きされている対象を記録する配列であり、空配列でなければCIを通過させない。空配列という自己申告と実画像が矛盾する場合は、進行編集が差し戻す。
...
```json
{
  ...
  "readerComfort": {
    "reviewedBy": "agent:visual-editor",
    "hygieneSensitiveItemsOnFloor": [],
    "placementNotes": "ノートと飲み物は清潔な窓辺のテーブル上にあり、床へ直接触れていない。"
  },
...
- 食品、飲料容器、書籍、衣類、ハンカチ、財布、鍵などが、路面や玄関土間を含む不衛生に見える床へ直接置かれていない。
- sidecarの `readerComfort` と実画像の置き場所が一致している。
