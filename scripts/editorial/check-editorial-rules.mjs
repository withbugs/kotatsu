...
requireText('docs/editorial/agent-workflow.md', '非実写調1カテゴリ');
requireText('docs/editorial/agent-workflow.md', '回復時に写真調へ単純化しない');
...
requireText('.agents/kotatsu/editor-in-chief.md', 'カテゴリ別の固定構図表を作らない');
...
requireText('.agents/kotatsu/managing-editor.md', '直前の同カテゴリhero');
...
requireText('docs/editorial/recovery-workflow.md', '## Source-of-Truth Recovery');
...
requireText('docs/editorial/recovery-workflow.md', 'pnpm recovery:actions');
requireText('docs/editorial/recovery-workflow.md', '`queued`から30分未満');
...
requireText('.codex/rules/kotatsu-scheduled-network.rules', 'scripts/editorial/monthly-planning-recovery.mjs');
requireText('.codex/rules/kotatsu-scheduled-network.rules', 'scripts/editorial/actions-recovery.mjs');
requireText('.codex/rules/kotatsu-scheduled-network.rules', '"pnpm", "recovery:actions"');
...
requireText('package.json', '"planning:recover"');
...
requireText('package.json', '"recovery:source-conflict"');
requireText('package.json', '"recovery:actions"');
